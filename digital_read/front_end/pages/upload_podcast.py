...
async def upload_podcast(page, navigate):
    ...
    def validate_content(media_name):
        try:
            ...
            if res.status_code == 200:
                # read response to get uuid_cover
                urls = res.json()
                UUID_podcast_name = urls["UUID_podcast_name"]

                submit_btn.disabled = False
                message.visible = True
                message.color = ft.Colors.BLUE_600
                message.value = "Upload is valid!"
                return {"UUID_podcast_name": UUID_podcast_name}
    ...
    async def upload_logic():
        try:
            ...
            if not stored_media:
                message.visible = True
                message.color = ft.Colors.RED_600
                message.value = "Select a podcast first!"
                return None
            ...
            # when the stored media exists
                ...
                if not validation_response:
                    message.visible = True
                    message.color = ft.Colors.RED_600
                    message.value = "Podcast validation failed!"
                    return None
                    ...
                    if media_url:
                        ...
                        if upload_success == True and verify_upload_response.status_code == 200:
                            ...
                            if save_media_database:
                                ...
                                welcome_text.value = "Add a cover photo to make your podcast more attractive!"
                                title.visible = False
                                theme.visible = False
                                level.visible = False
...
################################################################################################################################################
    ...
    # podcast picker
    # opens dialogue to get podcasts
    # define podcast picker control it will manage opening and uploading podcasts_picker
    ...
    async def on_cover_pick(e):
        ...
        # update the selected_podcast to show details about file selected or show message when not selected
    ...
    async def on_media_pick(e):
        ...
        # update the selected_podcast to show details about file selected or show message when not selected
        selected_podcast.value = stored_media.name if stored_media else None
    ...
    podcast = ft.TextButton(
        "podcast", icon=ft.icons.Icons.DESCRIPTION, on_click=on_media_pick)
    selected_podcast = ft.Text("NO podcast selected", expand=True,)
