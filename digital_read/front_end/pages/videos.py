...
async def videos(page, navigate):
    ...
    def access_action_button(info, page, navigate, user_id, username, content_id):
        # if access_type for the video is free then access button is changed and color to show that is free to continue to read,
        # when clicked go to the video_view page passing media id there
        if info["access_type"] == "free" or subscribed:
            return ft.Button(
                ...
                on_click=lambda e: page.run_task(
                    # route_name,video(content)_id, user_id, username,
                    navigate, f"/video_view/{info['id']}/{user['id']}/{user['username']}"
                )
            )
        else:
    ...
    def handle_video_action(e, action, info, page, navigate):
        ...
        if action == "COMMENT":
            ...
            # page.run_task(navigate, f"/video/{info['id']}/comments")
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            videos = response.get("videos", [])  # get videos data
            ...
            for info in videos:
                # check if video is free so that one has full accesss to free ones otherwise no access
                                                ...
                                                ft.Row(
                                                    ...
                                                    controls=[
                                                        like_icon := ft.IconButton(icon=ft.Icons.FAVORITE, icon_size=16,
                                                                                   ...
                                                                                   on_click=lambda e, i=info: handle_video_action(
                                                ...
                                                ft.Row(
                                                    ...
                                                    controls=[
                                                        ft.IconButton(icon=ft.Icons.COMMENT_OUTLINED,
                                                                      tooltip="comments",
                                                                      on_click=lambda e, i=info: handle_video_action(e, "COMMENT", i, page, navigate)),  # opens function that opens a dialogue
            ...
            details = ft.Container(
                ...
                content=ft.Column(
                    ...
                    controls=[
                        # container simple nav bar with button to upload a new video
                        ft.Container(
                            content=ft.Row(
                                controls=[
        ...
        return ft.View(
            route="/videos",  # route for videos
            controls=[
                top_bar(page, navigate,),  # topbar for digital_read
                details,  # content about digital read videos
